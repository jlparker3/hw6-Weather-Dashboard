    ...
    function displayWeather() {
        ...
        var response = $.ajax({
            ...
            $.ajax({
                ...
                //setting the uv index range colors
                // var getIndex = uvResponse;
                // var indexNumber = indexNumber.value;

                // if (getIndex > 0 && getIndex <= 6) {
                //     indexNumber.addclass("low");
                // } else if (getIndex > 3 && getIndex <=8) {
                //     indexNumber.addclass("medium");
                // } else if (getIndex > 6 && getIndex <= 11) {
                //     indexNumber.addclass("high");
                // } else {
                //     (getIndex > 8 && getIndex <= 10.99)
                //     indexNumber.addclass("veryHigh");
                // }

                // indexNumber.text(getIndex);
                // indexNumber.appendTo(uvEl);
                // $("#weatherContainer").append(uvEl)
        ...
        var response = $.ajax({
            ...
            for (let i = 0; i < days.length; i++) {
                // if (days[i].dt_txt.includes("00:00:00")) {
                    ...
                    var tempDisplay = $("<p>").addClass("card-text").text("Temperature:  " + getTemp + "° F");
                    var humidDisplay = $("<p>").addClass("card-text").text("Humidity:  " + getHumidity + "%");
